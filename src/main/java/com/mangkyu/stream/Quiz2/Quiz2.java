...
public class Quiz2 {
    ...
    public Map<String, Integer> quiz1() {
        ...
        return WORDS.stream().map(line -> line.substring(0, 1)).collect(Collectors.toMap(vo -> vo, vo -> 1, (oldValue, newValue) -> oldValue + newValue));
    ...
    public String quiz2() {
        ...
        return WORDS.stream().filter(line -> line.length() >= 2).map(line -> line.substring(0, 1).toUpperCase()).collect(Collectors.joining(" "));
    }
