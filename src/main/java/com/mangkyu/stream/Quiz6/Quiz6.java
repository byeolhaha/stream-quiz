...
public class Quiz6 {
    ...
    public Map<Boolean, List<Student>> quiz1() {
        return Arrays.stream(stuArr).filter(stu -> stu.getScore() < 150).collect(groupingBy(Student::isMale));
    }

    // 각 반별 총점을 학년 별로 나누어 구하여라 (Map<Integer, Map<Integer, Integer>>)
    public Map<Integer, Map<Integer, Integer>> quiz2() {
        return Stream.of(stuArr).collect(groupingBy(Student::getHak, groupingBy(Student::getBan, summingInt(Student::getScore))));
    }
}
